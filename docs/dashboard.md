...
The main consumer is the **[Claude Code brain](/paws-brain#claude-code-provider-local-cli)**: with `CLAUDE_CODE_EXPOSE_TOOLS=1`, Claude Code calls OpenVole's own tools as `mcp__openvole__<tool>` through this endpoint. The engine injects `VOLE_DASHBOARD_URL`, `VOLE_AGENT_ID`, and the token into each agent, so it wires up automatically under `vole serve`.
