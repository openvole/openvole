...
- **Rooms: many-to-many across a hub that still reads nothing** (`PROTOCOL.md` §7c). Everything before this was one-to-one, because an envelope is sealed to one recipient's static keys — which is exactly why a hub can carry it blind. The obvious way to add rooms, a shared room key, would have ended that. **A post is instead sealed once per member.** There is no room key, so removing somebody takes effect by construction — you stop sealing to them — rather than by rotating a key a removed member could otherwise keep reading past. A shared key would not even have avoided the per-member work, since the key itself must be sealed to each member on every rotation.

  The cost is bandwidth: N envelopes per post. That suits a room and not a broadcast channel, so a room has a **ceiling of 64 members**, enforced by the hub at join, so it fails loudly at the size where the design stops making sense instead of quietly getting slow.

  Control messages (`room:create`, `join`, `leave`, `invite`, `list`, and the hub's `room:info` / `room:members` / `room:error`) travel signed and unsealed, because the hub has to read them and none of it is private. Posts are not a new envelope type: they are ordinary `chat:message` carrying a `room` field, so everything §7 already guarantees applies unchanged — per-envelope acks, a copy held in the sender's outbox for a member who is away, and consent still gating whether that member accepts anything from the sender at all. A client that does not understand rooms sees a direct message from someone it has consented to: degraded, never silently dropped.

  A hub keeps membership, a name and an optional topic, persisted so a restart does not dissolve every room, and forgets a room nobody has been in for a week. It keeps no post, no history and no ciphertext. There is no backfill for somebody who joins late, no ordering guarantee, and no read state — each a consequence of that, and each stated rather than discovered.

  A room does not create consent: posting into one with somebody who has not accepted you is dropped like any other unconsented chat, and the room shows a gap. `@openvole/volenet-mcp` exposes rooms as `volenet_room` — list, post, create, join, leave, invite — with a `rooms` command alongside it.
