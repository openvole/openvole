...
| `relay` | `object` | — | Blind relay. Hub: `{ enabled, maxPerMinutePerPair (30), maxBytes (65536) }` — forward sealed member↔member envelopes the hub cannot read. Member: `acceptFrom` — who may reach you over a relay: unset = only peers you approve or already trust (default deny); `"*"` = any hub member; or a list of names / instanceId prefixes. v1 carries end-to-end encrypted chat only. Away handling: `outboxTtlHours` (168) — how long a message to an absent member waits in *your* outbox; `noticeTtlHours` (168) — hub side, how long a "somebody tried to reach you" notice is kept. Neither store holds anything readable; see [When the other side is away](/volenet#when-the-other-side-is-away). See [Relay](/volenet-relay). |
