...
import { buildPermissionFlags, computeEffectivePermissions } from '../../src/paw/sandbox.js'
import type { EffectivePermissions, PawConfig, PawManifest } from '../../src/paw/types.js'
...
describe('computeEffectivePermissions', () => {
	...
	it('returns empty arrays when nothing granted', () => {
		...
		const config = makeConfig({
			allow: { network: [], listen: [], filesystem: [], env: [], childProcess: false },
		})
		const result = computeEffectivePermissions(manifest, config)
...
describe('buildPermissionFlags', () => {
	...
	it('adds filesystem permissions to both read and write', () => {
		...
		const readPaths = flags
			.filter((f) => f.startsWith('--allow-fs-read='))
			.map((f) => f.replace('--allow-fs-read=', ''))
		const writePaths = flags
			.filter((f) => f.startsWith('--allow-fs-write='))
			.map((f) => f.replace('--allow-fs-write=', ''))
