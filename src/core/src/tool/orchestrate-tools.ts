...
/**
 * The one sibling tool every agent gets, orchestrator or not.
 *
 * Messaging is not orchestration. Talking to a colleague is what any agent should be able to do;
 * assigning them work, rewriting their identity or restarting them is management, and that whole
 * `agent_*` family stays behind the orchestrator flag. Both travel the same reverse-RPC channel, so
 * the split is drawn by intent rather than by transport — here, and again in the control plane.
 */
