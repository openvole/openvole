...
import { aead, aeadOpen } from './node-crypto.js'
...
export function encryptChunk(
	...
	const cipher = aead(
		crypto.createCipheriv('chacha20-poly1305', key, chunkNonce(index), {
...
export function decryptChunk(
	...
	try {
		const decipher = aeadOpen(
			crypto.createDecipheriv('chacha20-poly1305', key, chunkNonce(index), {
				authTagLength: TAG_BYTES,
			}),
		)
