...
import { aead, aeadOpen, mlkem } from './node-crypto.js'
...
export function seal(
	...
	if (recipientMlkemPubB64) {
		...
		if (mlkemPub) {
			try {
				const { sharedKey, ciphertext } = mlkem(crypto).encapsulate(mlkemPub)
	...
	const cipher = aead(
		crypto.createCipheriv('chacha20-poly1305', key, nonce, { authTagLength: TAG_BYTES }),
	)
...
export function unseal(
	...
	try {
		...
		if (box.kem) {
			...
			const ssPq = mlkem(crypto).decapsulate(mlkemPrivateKey, Buffer.from(box.kem, 'base64'))
		...
		const decipher = aeadOpen(
			crypto.createDecipheriv('chacha20-poly1305', key, nonce, { authTagLength: TAG_BYTES }),
		)
