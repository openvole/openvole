...
import { type EventBus as MessageBus, createEventBus as createMessageBus } from '../src/index.js'
...
describe('consent-based pairing', () => {
	...
	it('queues a pair request without trusting, then accept grants trust live', async () => {
		...
		const { trustPeer } = await import('../src/keys.js')
	...
	it('dashboard flow: probePair + initiatePair pair two fresh nodes live', async () => {
		...
		// Remembering a learned peer belongs to the host, which owns its own config format —
		// the library only says when. This plays that host, the way OpenVole's engine does.
		const configFile = path.join(rootC, 'vole.config.json')
		await fs.writeFile(configFile, JSON.stringify({ net: {} }))
		const persistPeer = async (url: string) => {
			const cfg = JSON.parse(await fs.readFile(configFile, 'utf-8')) as {
				net?: { peers?: Array<{ url: string }> }
			}
			cfg.net = cfg.net ?? {}
			cfg.net.peers = cfg.net.peers ?? []
			if (!cfg.net.peers.some((p) => p.url === url)) cfg.net.peers.push({ url })
		...
		try {
			...
			// The host was asked to remember it, so it survives a restart.
			const cfg = JSON.parse(await fs.readFile(configFile, 'utf-8'))
			expect(cfg.net.peers.some((p: { url: string }) => p.url === 'http://127.0.0.1:19974')).toBe(
				true,
			)
		} finally {
